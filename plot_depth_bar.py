...
def parse_depth_or_fog_from_name(path):
    """Extract depth (preferred) or fog value from filename.

    Supports patterns like:
      - _depth_<decimal> (e.g., _depth_11.0)
      - log_depth_<decimal> (e.g., log_depth_11.0)
      - depth<decimal> (fallback)
      - _fog_<decimal>, fog<decimal> (backward compatibility)
    """
    base = os.path.basename(path)

    # Prefer depth patterns
    m = re.search(r'_depth_(\d+\.?\d*)', base)
    if m:
        return float(m.group(1))

    m = re.search(r'log_depth_(\d+\.?\d*)', base)
    if m:
        return float(m.group(1))

    m = re.search(r'depth(\d+\.?\d*)', base)
    if m:
        return float(m.group(1))

    # Backward compatibility: fog patterns
...
def main():
    parser = argparse.ArgumentParser(description='Line plot: Depth vs avg depth error (cm) and confidence with customizable σ error bars.')
    parser.add_argument('--pattern', default='eval_output/depth_error_log_depth_*.csv', help='Glob pattern for CSV files')
    parser.add_argument('--out', default='eval_output/depth_error_depth_line.png', help='Output PNG path')
    parser.add_argument('--title', default='Average Absolute Depth Error and Confidence vs Depth', help='Plot title')
    parser.add_argument('--figsize', nargs=2, type=float, default=[14, 8], help='Figure size as width height (default: 14 8)')
    parser.add_argument('--sigma-multiplier', type=float, default=2.0, help='Sigma multiplier for error bars (e.g., 1.0 for 1σ, 2.0 for 2σ, 0.5 for 0.5σ)')
    ...
    data = []  # (depth_value, avg_mean_cm, sigma_cm, conf_mean, conf_sigma)
    for fp in files:
        depth_value = parse_depth_or_fog_from_name(fp)
        if depth_value is None:
            print("Warning: Could not parse depth from {}, skipping".format(fp))
            continue
        avg_mean, sigma_cm, conf_mean, conf_sigma = load_csv_stats(fp)
        if np.isfinite(avg_mean) and np.isfinite(sigma_cm):
            data.append((depth_value, avg_mean, sigma_cm, conf_mean, conf_sigma))
    ...
    depth_err = [args.sigma_multiplier * d[2] for d in data]  # Customizable σ
    conf_y = [d[3] for d in data]
    conf_err = [args.sigma_multiplier * d[4] if np.isfinite(d[4]) else np.nan for d in data]
    ...
    ax1.fill_between(x, 
                     ...
                     label='Depth Error ±{:.1f}σ (dotted)'.format(args.sigma_multiplier))
    ...
    # Set x-axis ticks to show depth values
    ax1.set_xticks(x)
    ...
    if any(np.isfinite(conf_y)):
        ...
        ax2.fill_between(x, 
                         ...
                         label='Confidence ±{:.1f}σ (dashed)'.format(args.sigma_multiplier))
    ...
    # Update title to include sigma information
    title_with_sigma = '{} (±{:.1f}σ error bars)'.format(args.title, args.sigma_multiplier)
    ...
    print("\nProcessed {} depth experiments:".format(len(data)))
    print("Using ±{:.1f}σ error bars".format(args.sigma_multiplier))
    for d in data:
        depth_value, avg_mean, sigma_cm, conf_mean, conf_sigma = d
        conf_text = ', conf_mean={:.3f}, conf_{:.1f}sigma={:.3f}'.format(conf_mean, args.sigma_multiplier, args.sigma_multiplier*conf_sigma) if np.isfinite(conf_mean) and np.isfinite(conf_sigma) else ''
        print('depth_{:.1f}: mean={:.3f} cm, {:.1f}sigma={:.3f} cm{}'.format(depth_value, avg_mean, args.sigma_multiplier, args.sigma_multiplier*sigma_cm, conf_text))
